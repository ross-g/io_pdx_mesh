...
try:
    ...
    pathlib.Path(numpy_maya).mkdir(exist_ok=True)
    ...
    try:
        ...
        # pip is included with Maya 2022+, but needs to be installed otherwise
        try:
            import pip

            EXTERNAL_LOG.info("Found pip at: {}".format(path.dirname(pip.__file__)))
            ...
            # try installing pip (after downloading get-pip.py)
            try:
                # first get pip working
                get_pip = download_get_pip()
                subprocess.check_call([mayapy, get_pip], creationflags=subprocess.CREATE_NEW_CONSOLE)
                import pip
        ...
        # since Maya 2022 (Py3) numpy can just be installed via pip
        if six.PY3 and path.isfile(mayapy):
            try:
                subprocess.check_call(
                    [mayapy, "-m", "pip", "install", "numpy"], creationflags=subprocess.CREATE_NEW_CONSOLE
                )
                import numpy  # noqa
        ...
        # otherwise try installing numpy from a bespoke wheel
